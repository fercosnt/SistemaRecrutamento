...
**Objetivo:** deixar `rh.beautysmile.com.br` verificado no Resend (SPF + DKIM automáticos), com o DMARC publicado manualmente, para que os e-mails transacionais do ATS cheguem na **Caixa de entrada** do candidato — não no spam.
...
| Item | Valor |
|------|-------|
| Domínio de envio | `rh.beautysmile.com.br` |
| Região do Resend | `sa-east-1` (São Paulo) |
| From | `Beauty Smile Recrutamento <nao-responda@rh.beautysmile.com.br>` |
| Reply-To | `rh@beautysmile.com.br` |
| DMARC — nome | `_dmarc.rh.beautysmile.com.br` |
...
> ℹ️ Por que um subdomínio e não `beautysmile.com.br` direto: isola a reputação de envio do domínio corporativo. Se um envio do ATS gerar reclamação, o e-mail humano da empresa não é afetado. O subdomínio já é usado pelo produto (link público de vaga), e os records do Resend ficam em `send.rh.…` e `<token>._domainkey.rh.…` — **não colidem** com o A record do site.
...
2. Nome: `rh.beautysmile.com.br`.
3. Região: **`sa-east-1` (São Paulo)**. Ver o aviso acima antes de confirmar.
4. Return-path: deixar no **default (`send`)**. É literalmente daí que sai o subdomínio `send.rh.beautysmile.com.br` do MX/SPF. Não use valores criativos tipo `testing` — reduzem credibilidade do envelope.
...
|-------|-----------------------------------------------------|------|-----------------|
| SPF (return-path) | `send.rh.beautysmile.com.br` | `MX` | dashboard — aponta para `feedback-smtp.sa-east-1.amazonses.com`, prioridade `10` |
| SPF | `send.rh.beautysmile.com.br` | `TXT` | dashboard — `v=spf1 include:amazonses.com ~all` |
...
> O que o dashboard exibe já embute o `rh` (o domínio adicionado no Resend é o **subdomínio inteiro**, mas a zona que você edita é `beautysmile.com.br`). Existem então três formas do mesmo nome, e você precisa saber qual cada campo quer:
> - **FQDN final** — `send.rh.beautysmile.com.br`. É o que o `dig` do Passo 4 recebe e o que alguns painéis (Registro.br) exigem no campo *Name*.
> - **Nome relativo à zona editada** — `send.rh`. É o que a Cloudflare quer (ela re-acrescenta `beautysmile.com.br` sozinha — ver § 3.1).
> - **Nome exibido pelo dashboard do Resend** — pode ser qualquer uma das duas formas acima, dependendo da conta/época.
>
> **Regra única que fecha os três casos:** o FQDN final tem que conter o rótulo `rh` **exatamente uma vez**. Se você contou `rh` duas vezes (`…rh.rh.beautysmile.com.br`), concatenou o sufixo em cima de um nome que já o tinha.
...
- **Omita o sufixo do domínio no campo *Name***. A Cloudflare re-acrescenta a zona automaticamente. Colar `send.rh.beautysmile.com.br` produz `send.rh.beautysmile.com.br.beautysmile.com.br`. Cole apenas `send.rh` (ou o que a Cloudflare mostrar como resultado no preview do campo).
...
- Alguns formulários exigem o **FQDN completo** no campo de nome (`send.rh.beautysmile.com.br.`, às vezes com ponto final) em vez do nome relativo que o Resend mostra. Não há regra universal.
...
| Campo | Valor |
|-------|-------|
| Nome | `_dmarc.rh.beautysmile.com.br` |
| Tipo | `TXT` |
...
```bash
# SPF / return-path
dig +short MX  send.rh.beautysmile.com.br
dig +short TXT send.rh.beautysmile.com.br

# DKIM — NÃO acrescente sufixo às cegas. Monte o FQDN final e CONFIRA antes de rodar:
# o FQDN tem que conter o rótulo "rh" EXATAMENTE UMA VEZ (ver a regra do § 2).
#
#   caso 1 — o dashboard já mostra o FQDN completo (contém "beautysmile.com.br"):
#            use o nome COMO ESTÁ, sem concatenar nada.
#            ex.: dashboard mostra "resend._domainkey.rh.beautysmile.com.br"
dig +short CNAME resend._domainkey.rh.beautysmile.com.br
#
#   caso 2 — o dashboard mostra o nome relativo à ZONA que você editou (contém
#            "rh", mas não "beautysmile.com.br"): acrescente ".beautysmile.com.br".
#            ex.: dashboard mostra "resend._domainkey.rh"
dig +short CNAME resend._domainkey.rh.beautysmile.com.br
#
#   caso 3 — o dashboard mostra um nome SEM "rh": aí sim o sufixo é o
#            domínio de envio inteiro.
dig +short CNAME <nome-do-dashboard>.rh.beautysmile.com.br
#
...
dig +short TXT _dmarc.rh.beautysmile.com.br
```

Cada comando deve devolver **exatamente** o valor colado no painel. Sinais de erro comuns:

- **FQDN com `rh.rh`** (ou com `beautysmile.com.br` duas vezes) → você concatenou o sufixo em cima de um nome que já o tinha. O erro está no **comando `dig`**, não no DNS: corrija o FQDN e rode de novo. **NÃO mexa nos records** — ver o aviso no fim desta seção.
- Resposta vazia → **primeiro releia o FQDN que você digitou** (regra do § 2: rótulo `rh` exatamente uma vez). Só depois de o nome estar comprovadamente certo é que "resposta vazia" significa "record ainda não propagou" ou nome errado no painel (sufixo duplicado no campo *Name* — ver § 3.1).
...
2. **DEPOIS de verificado:** o From passa a ser `nao-responda@rh.beautysmile.com.br` e o destino pode ser `delivered@resend.dev` (ou `delivered+<evento>@resend.dev`, que é o formato usado pelo modo `teste` do `_shared/email-config.ts`). A restrição é do **domínio remetente**, não do destinatário.
