...
ci_fully_green: "Live CI run 27076233734 (commit 3cdad46) is the first FULLY GREEN pipeline on backup/local-state-2026-04: unit + e2e + lighthouse all success. The pre-existing LoadingProgress unit carryover (out-of-Phase-05) was fixed in 3cdad46 (test corrected to assert the unique error-message paragraph rather than counting all .text-destructive nodes; component behavior unchanged; vitest 358/358; tsc baseline unchanged at 292). HARD-01 now has a real green check."
gaps:
  - id: GAP-05-CI-1
    sc: SC2 / HARD-01
    severity: blocking
