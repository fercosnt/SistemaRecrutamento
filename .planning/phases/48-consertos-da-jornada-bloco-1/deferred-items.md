...
- Rodapé da devolutiva: «Conteúdo revisado por psicólogo(a) responsável» e a menção negada a «teste psicológico» (observação do operador na sessão 1 do 48-18)
  status: parcialmente decidido — **2026-09-22, operador: MANTER a frase «não é teste psicológico»** (a forma negada é exigida pelo produto, como o comentário do `_NEG` na EF já dizia). **2026-09-22, operador: REMOVER a frase de supervisão profissional** («não há revisão») — feito no código em `aa2b38ed` (EF + fallback do front + `templates-devolutiva.md`), **aguardando o deploy da EF v26 e o push**; as 3 devolutivas já gravadas (teste) guardam o rodapé antigo. Seguem abertos: o termo em inglês «self-assessment» (observação do operador na sessão 2) e a promessa «Você pode solicitar explicação detalhada ou revisão humana a qualquer momento», que a página da devolutiva não cumpre por nenhum caminho (achado da verificação da fase) — **decidido 2026-09-22 pelo operador: REMOVER** (opção a); feito no código, aguardando o deploy da EF v27 e o push
