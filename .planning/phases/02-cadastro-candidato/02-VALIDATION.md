...
| 2-02-T1 | 02 | 1 | CAD-03, CAD-05 | T-02-01, T-02-02, T-02-03, T-02-06 | Migration authors hybrid rate-limit (x-forwarded-for + composite hash_cpf_email) + pgcrypto extension + autorizacoes cols (policy_version/ip_aceite/user_agent_aceite/user_id, no data_aceite) | schema | `f=supabase/migrations/20260421000001_rate_limit_duplicate_check.sql && test -f $f && grep -q "CREATE EXTENSION IF NOT EXISTS pgcrypto" $f && grep -q "CREATE TABLE IF NOT EXISTS public.rate_limit_check_duplicate" $f && grep -q "x_forwarded_for" $f && grep -q "hash_cpf_email" $f && grep -q "rate_limited" $f && grep -q "ADD COLUMN IF NOT EXISTS policy_version" $f && grep -q "ADD COLUMN IF NOT EXISTS user_agent_aceite" $f && grep -q "'v1.0-2026-04'" $f && grep -q "idx_autorizacoes_user_id" $f && grep -q "idx_autorizacoes_policy_version" $f && grep -q "GRANT EXECUTE ON FUNCTION public.check_candidato_duplicate(text, text) TO anon, authenticated" $f && grep -q "REVOKE ALL ON TABLE public.rate_limit_check_duplicate FROM anon" $f && ! grep -q "inet_client_addr" $f && ! grep -q "data_aceite" $f` | ❌ W0 (creates) | ⬜ pending |
