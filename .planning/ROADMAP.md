...
| 4.1. Auth Hydration Fix (INSERTED) | 4/5 | Wave 0+1+2+3 complete. Plan 01 (Wave 0): 6 test artifacts; 9 RED+GREEN assertions; tsc 320→296 (−24 via jest-dom shim). Plan 02 (Wave 1): hydrateFromSession async action + waitForCandidatoHydrated utility + App.tsx setTimeout(0) listener (Pattern 1) + RoleGuard fallbackTriedRef one-shot DB fallback (Pattern 3); 5 RED→GREEN flips (4 hydrateFromSession + 1 RoleGuard one-shot); 2 found12 RED held for Plan 04; tsc 296 preserved; build green; commits a873128 + 332364f + 4d9fa25. Plan 03 (Wave 2): defense-in-depth submit handlers consume waitForCandidatoHydrated; LoginCandidatoPage + RedefinirSenhaPage [happy path + Pitfall 2 fallback] + CadastroMultiStepForm Step 4 = 7 occurrences across 3 files; FormularioCandidaturaPage silent-return guard (`if (!cvFile || !user || !candidato || !vaga) return`) replaced by 3 distinct pt-BR toasts (session-not-hydrated / no-CV / no-vaga) + inline `disabled={!candidato || !cvFile || ...}` on submit button (removed unused `submitDisabled` local); submit happy path UNCHANGED (uploadCV + submitCandidaturaWithRespostas count = 6 = pre-task count); 0 new RED→GREEN (Plan 02 covered the underlying mechanism; Plan 03 is Pattern 2 application — grep + tsc + Pitfall 7 substitute for net-new fixtures); all Wave 0/1 GREEN tests preserved GREEN; tsc 296 preserved; build green; commits aec3e27 + 1534b45. **Closes FLOW-CADASTRO + FLOW-RECOVERY + FLOW-CANDIDATURA at the page layer.** Plan 04 (Wave 3 — FOUND-12 literal close): src/store/adminAuthStore.ts DELETED (228 LoC re-export shim, 12 exported symbols of which 10 had zero real consumers — 90% dead surface confirmed dead at deletion time); 2 real consumers migrated (App.tsx inline useIsAdminAuthenticated selector + useSessionTimeout.ts direct useAuthStore import + JSDoc @example sync); LoginRHPage D-14 doc-comment rewritten to drop 'antigo adminAuthStore' literal + add 'deletado em Phase 4.1 — FOUND-12' historical pointer + replace obsolete 'setSession' mention with 'hydrateFromSession (Phase 4.1 fix)'; pre-delete consumer audit returned 0 matches (Phase 3 [03-06] precedent recurrent); 2 found12 RED → GREEN flip — **Wave 0 RED→GREEN battery now COMPLETE (7/7 flips)**; INT-WARNING-2 closed (REQUIREMENTS.md text 'deletado' matches working tree literally); commits 0a2ff71 + 8005fd5. Wave 4 next: 04.1-05-PLAN.md UAT runbook + phase verification battery (lint baseline 296 + build + Vitest + Playwright real-login + 04.1-VERIFICATION.md tracing 7 audit gaps + 6 ROADMAP success criteria). | - |
