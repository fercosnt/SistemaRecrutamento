...
- [x] 04.1-02-PLAN.md — Wave 1: core hydration fix (hydrateFromSession action + waitForCandidatoHydrated utility + App.tsx setTimeout(0) listener + RoleGuard one-shot fallback for INT-WARNING-3)
...
| 4.1. Auth Hydration Fix (INSERTED) | 2/5 | Wave 0+1 complete. Plan 01 (Wave 0): 6 test artifacts; 9 RED+GREEN assertions; tsc 320→296 (−24 via jest-dom shim). Plan 02 (Wave 1): hydrateFromSession async action + waitForCandidatoHydrated utility + App.tsx setTimeout(0) listener (Pattern 1) + RoleGuard fallbackTriedRef one-shot DB fallback (Pattern 3); 5 RED→GREEN flips (4 hydrateFromSession + 1 RoleGuard one-shot); 2 found12 RED held for Plan 04; tsc 296 preserved; build green; commits a873128 + 332364f + 4d9fa25. Wave 2 next: defense-in-depth submit handlers (Login/Redefinir/Cadastro await waitForCandidatoHydrated + FormularioCandidatura silent-return fix). | - |
