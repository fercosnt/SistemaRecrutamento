...
- [x] 41-02-PLAN.md — EF `resend-webhook` (Svix verify + reconciliação por `provider_message_id`) + `config.toml` verify_jwt=false [wave 1]
- [x] 41-03-PLAN.md — migration aditiva (`bounce_em`/`reclamado_em` + `ler_resend_webhook_secret` + `varrer_retry_notificacoes` + cron `notif-retry-sweep`) + smoke gate-GUC [wave 1]
