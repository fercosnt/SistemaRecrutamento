...
- [x] **INSCR-04**: Auto-rejeição imediata ao marcar opção `tag='knockout'` (`status='rejeitado'`, `etapa='inscricao'`, `motivo='knockout_automatico'`, `opcao_knockout_id`) + mensagem ao candidato + linha de auditoria em `historico_candidatura` (`auto_rejeitado=true`) [RF-03, RF-04]
...
- [x] **AVAL-07**: Revisão humana sempre obrigatória pós-IA (status `pendente_humano`) — UI 1-redação-por-vez, sidebar por cor, sliders override, `notas_revisor ≥50 chars`, `decisao_revisor` (aprovado/reprovado/duvida); "duvida" escala ao gestor [RF-17a]
...
- [x] **ENTREV-02**: Tela do gestor `/rh/candidato/:id/entrevista` mostra dashboard do candidato + guia + scorecard inline editável (`notas_humanas`); notificação ao gestor 24h antes (agendamento manual no V1 — auto-scheduling deferido, ver Future) [RF-22, RF-20c]
...
- [x] **DECISAO-03**: Decisão final exige justificativa textual obrigatória ≥50 caracteres → `decisao_final` com `por_usuario` NOT NULL + `decisao` enum (aprovado/rejeitado/em_espera) + DB constraints [RF-31]
...
- [x] **IA-02**: Logging estruturado obrigatório de toda chamada IA (`prompt_version`, `model_version`, `generated_at`, `input_hash`, `output`, `custo_tokens`), auditável retroativamente em SQL [RNF-09]
- [x] **IA-03**: Anthropic prompt caching (ephemeral) nas partes estáveis do contexto (vaga + rubric + few-shot) + mix Haiku/Sonnet, mantendo custo médio ≤ R$ 0,50 por candidato no funil completo [RNF-10, §8.4]
