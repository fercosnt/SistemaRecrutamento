...
- [x] **KPI-02**: RH vê KPIs operacionais essenciais sobre `historico_candidatura` — tempo **mediano** por etapa, taxa de conversão etapa-a-etapa, volume por vaga/etapa — computados por RPC SECURITY DEFINER **vaga-scoped** (nunca agregação client-side, nunca PII).
- [x] **KPI-03**: RH vê um indicador de aging/SLA breach (candidatos parados além do limite configurado por etapa).
- [x] **KPI-04**: RH vê KPIs adicionais no mesmo RPC — time-to-hire, taxa de knockout, drop por etapa e taxa de no-show (esta habilitada por AGEND-03).
