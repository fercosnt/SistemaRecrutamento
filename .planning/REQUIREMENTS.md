...
| AUTH-01 | Phase 3: Login + Recuperacao | In progress — Partial (03-02 + 03-03, 2026-04-25): error taxonomy (AuthError 6-code union + isAuthError guard) + Supabase-error mapping (mapSupabaseError + extractRetryAfterSeconds clamp-to-3600s) shipped in src/features/auth/{types,utils}/; loginSchema (email+password+rememberMe) shipped in src/features/auth/schemas/. Role extraction primitive landed in 03-03 — `extractRole(session)` reads JWT payload via jwt-decode@^4 (Bug 1 / D-13 CLOSED); canonical `Role` type exported from `@/features/auth/utils`. Business flow (signIn + page rewrite + error toasts) pending Wave 3 (Plans 03-04, 03-05) |
| AUTH-02 | Phase 3: Login + Recuperacao | In progress — Partial (03-03, 2026-04-25): storage strategy primitives landed — `rememberMeStorage` adapter + `setRememberMeMode(mode)` in src/features/auth/utils/; supabase-js singleton at src/lib/supabase/client.ts now uses `storage: rememberMeStorage` with late-binding currentMode flag and symmetric sb-* wipe on swap (T-03-04 mitigated). Business flow (checkbox wired to `setRememberMeMode(rememberMe ? 'local' : 'session')` before `signInWithPassword`) pending Wave 3 (Plan 03-04 authService.signIn) |
...
*Last updated: 2026-04-25 after Phase 3 Wave 2 (03-03) — AUTH-02 moved to In progress (rememberMeStorage + setRememberMeMode + supabase client wire-up); AUTH-01 annotation extended with role-extraction primitive (extractRole jwt-decode closing Bug 1 / D-13); AUTH-03 + AUTH-04 unchanged; business flows still pending Waves 3-5*
