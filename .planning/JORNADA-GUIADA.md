...
| **30** | 48-18 (prova, sessão 1) | 🔴 **A devolutiva do Big Five inventava nome, percentil e cargo.** O prompt `bigfive_devolutiva` v1.0.0 manda personalizar e o bloco de usuário só leva a faixa e o texto oficial: a IA escreveu «Rodrigo Fonseca», «percentil 12», «Candidato não informado», «[NOME COMPLETO DO CANDIDATO]», na terceira pessoa. Só apareceu quando a página passou a existir (conserto do 6) | 4 de 5 páginas da `+claude5` fora do template; nenhum candidato com esse nome; o nome só na resposta. Alcance: 2 devolutivas em PROD, ambas de teste · ✅ **consertado e provado em PROD** (48-19: texto oficial da faixa, IA desligada; `+claude6` com as 5 páginas = template byte a byte, md5) |
| **31** | 48-18 (prova, sessão 1) | **A aba «Sensibilidade Emocional» da devolutiva não recebia clique.** O `TabsList` base tem altura fixa; a 2ª linha transbordava para baixo do cartão. Em 375 px eram **4 de 5** abas cobertas | medido no navegador com o componente real (`elementFromPoint`) · ✅ **consertado e provado em PROD** (48-19, só na view; `+claude6`: as 5 abas abrem) |
| **27** | 12 | 🔴 **A titular não é avisada de nada sobre os próprios dados.** Pedido de exclusão e cancelamento geraram 3 e-mails, todos para o RH, zero para ela. Sem isso, conta invadida → exclusão pedida → ela não sabe | `notificacoes_enviadas` 20:38:26: rh2, rh3, fernando@. `recibo_enviado_em` null · ✅ **consertado e provado em PROD** (Phase 48, `48-07`; `p48_prova_prod.sql` 17/17 em 2026-09-22) |
| **26** | 12 | **O pedido de exclusão marcou «encerrada a pedido» uma candidatura eliminada por knockout** — o filtro de «em andamento» olha `etapa_atual` e ignora `status`. **Mesmo eixo do Defeito 22** | `encerrada_a_pedido_em` preenchida na Social Media (`etapa_atual='inscricao'`) e null na Consultor (`etapa_atual='rejeitado'`) · ✅ **consertado e provado em PROD** (Phase 48, `48-01/48-02/48-12`; `p48_prova_prod.sql` 17/17 em 2026-09-22) |
...
| **22** | 9 | 🔴🔴 **O Art. 20 é inalcançável para quem é rejeitado na triagem.** O cartão «Ver explicação» some: a condição exige `data_decisao_final OR feedback_rejeicao`, e a rejeição humana grava `motivo_rejeicao`/`etapa_justificativa` — nenhum dos dois | `DashboardCandidatoPage.tsx:159`; medido: os dois campos null. Backend cobre os 3 casos (Phase 46), o front gateia pelo critério da Phase 17 · ✅ **consertado e provado em PROD** (Phase 48, `48-09/48-12`; `p48_prova_prod.sql` 17/17 em 2026-09-22) |
| **20** | 9 | 🔴 **Rejeitar na triagem não avisa o candidato.** A RPC `rejeitar_candidatura` não dispara notificação nenhuma — sem `net.http`, sem evento. A rejeição mais comum de todas é silenciosa por construção | `notificacoes_enviadas` sem linha nova; a RPC (`20260714100001`) não tem caminho de despacho · ✅ **consertado e provado em PROD** (Phase 48, `48-08/48-09`; `p48_prova_prod.sql` 17/17 em 2026-09-22) |
...
| **19** | 8 | **«Revertida» não reverte.** O veredito é gravado e `decisao`, `etapa_atual` e `status` seguem `rejeitado`. O e-mail diz à candidata que «a decisão foi revista» e o painel dela diz Rejeitado. Não há caminho para executar a reversão | `revisao_veredito='revertida'` × `decisao='rejeitado'`; a RPC só grava o veredito (REVISAO-03) · ✅ **consertado e provado em PROD** (Phase 48, `48-11/48-13/48-14/48-15`; `p48_prova_prod.sql` 17/17 em 2026-09-22) |
| **18** | 8 | 🔴 **A candidata foi rejeitada e não foi avisada.** O `dedupe_key` é `<candidatura>:decisao`, sem a decisão nem o instante: a aprovação ocupou a chave e a rejeição foi descartada em silêncio | 3 e-mails saíram (todos p/ RH), 0 para ela. A linha antiga segue `entregue`; nada no banco denuncia · ✅ **consertado e provado em PROD** (Phase 48, `48-08`; `p48_prova_prod.sql` 17/17 em 2026-09-22) |
...
| **15** | 7 | **O sistema promete avisar «a cada etapa» e avisa em 1 de 4.** A candidata foi movida três vezes em silêncio | Texto do e-mail de confirmação × `notificacoes_enviadas` · ✅ **consertado e provado em PROD** (Phase 48, `48-10/48-16`; `p48_prova_prod.sql` 17/17 em 2026-09-22) |
...
> **✅ STATUS 2026-09-22 — BLOCO 1 CONSERTADO E PROVADO EM PROD.** Phase 48 (19 planos),
> provada pela consulta `supabase/tests/p48_prova_prod.sql` — **17/17 `true`** com as contas
> `+claude` e duas sessões do operador (`.planning/phases/48-consertos-da-jornada-bloco-1/48-PROVA-PROD.md`).
> Entraram também a parte antecipada do D5 (link obrigatório e validado, 48-03) e a U2 (link de
> login em todo e-mail, 48-07/48-16). A prova achou **dois defeitos novos** (30 e 31, na devolutiva
...
|---|---|---|
| **22** | Art. 20 inalcançável para quem é rejeitado na triagem | `rejeitar_candidatura` passa a gravar `feedback_rejeicao` (é o que o knockout já faz). **Conserto confirmado por experimento na Etapa 10** · ✅ **48-09/48-12** |
| **26** | Exclusão marca «encerrada a pedido» candidatura que o knockout eliminou | **Mesma família do 22.** Varrer pela FORMA: todo lugar que usa `etapa_atual` para saber se a candidatura acabou tem de olhar `status` · ✅ **48-01/48-02/48-12** |
| **20** | Rejeitar na triagem não avisa o candidato | `rejeitar_candidatura` não tem despacho. Acrescentar evento + template · ✅ **48-08/48-09** |
| **27** | Titular não é avisada de pedido/cancelamento de exclusão | Notificar o titular; `recibo_enviado_em` existe e não é escrito. **É controle de conta invadida, não cortesia** · ✅ **48-07** |
| **18** | 2ª decisão não gera aviso — `dedupe_key = <candidatura>:decisao` | Incluir a decisão e/ou o instante na chave · ✅ **48-08** |
| **15** | «Avisaremos por e-mail a cada etapa» — avisa em 1 de 4 | Ou avisa em todas, ou muda a promessa do e-mail · ✅ **48-10/48-16** |
| **24** | IA analisa quem o knockout já eliminou (US$ 0,045 cada) | Duas partes: **(a)** parar de analisar após knockout; **(b)** decidir o que fazer com o passivo já gerado, que sai na cópia LGPD · ✅ **48-04** |
| **6** | Devolutiva do Big Five 401 desde 2026-07-07 | **Causa NÃO provada.** Trocar `functions.invoke` por `fetch` explícito, ou instrumentar antes · ✅ **48-05/48-19** |
| **19** | «Revertida» não reverte nada | ⚠ **Decisão de produto antes do código** · ✅ **48-11/48-13/48-14/48-15** |
| **30** | Devolutiva inventava nome, percentil e cargo (achado na prova) | Servir o texto oficial da faixa, IA desligada até prompt corrigido e testado (decisão do operador) · ✅ **48-19** |
| **31** | Aba «Sensibilidade Emocional» sem clique (achado na prova) | Abas clicáveis e legíveis só na view, sem mexer no componente base (decisão do operador) · ✅ **48-19** |
