...
| 60 | 49 | stub | supabase/functions/gerar-guia-entrevista/index.ts | 340 | Teto de custo diário estourado: callAi devolve parsed={recommendation:'hold'} (não-null), então persistFlags fica VAZIO e a EF persiste um guia com 0 perguntas e nenhuma flag, devolvendo {ok:true}. Um roteiro barrado por gasto é indistinguível de um roteiro vazio bem-sucedido. MEDIDO pelo teste novo do 49-24 (needs_human=false no log). Proveniência NULL/NULL é honesta mas conflate com os 5 guias legados. Fora do escopo do 49-24 (proveniência); o conserto é a flag. | fixed | Consertado pelo plano 49-25 (EF gerar-guia-entrevista v20, ACTIVE em PROD): o helper guiaDeResultado() passou a perguntar pelo PROVEDOR (provider === 'none' => nenhum modelo respondeu) em vez de testar parsed == null, de modo que o teto de custo AI-06, a deteccao de injecao RF-PL-18 e todo bloqueio pre-provedor futuro ficam cobertos por construcao, sem lista literal de error_code. A linha passa a sair com incompleto:true e o error_code em flags; as perguntas origem:'manual' seguem preservadas pelo merge ENTREV-08, asserido. Provado por 5 mutacoes, todas mordendo. Medido em PROD: ZERO linhas ja gravadas no estado defeituoso e ZERO linhas provider='none' em ai_call_logs desde sempre (maior gasto diario por vaga da historia: US$ 0,3096 contra teto de US$ 50), logo o defeito era LATENTE e nao houve nada retroativo a corrigir. Razao escrita a mao: `windows fixed` aceita o motivo e o descarta (PATTERNS 49 §M). | 2026-09-23T01:40:15.497Z | 2026-09-23T04:41:36.084Z |
...
| 63 | 49 | deviation | .planning/phases/49-consertos-da-jornada-bloco-2/49-10-SUMMARY.md |  | 49-10: git push origin main NEGADO pelo ambiente. 2 migrations em PROD + EF v17 no ar com os commits NAO ENVIADOS (modo de falha do CLAUDE.md sobre os dois canais). Acao do operador: push + conferir origin/main..HEAD vazio. | fixed | Resolvido no proprio plano 49-10, em 2026-09-23. A primeira tentativa de `git push origin main` foi negada pelo ambiente e este registro foi aberto enquanto era verdadeiro; a tentativa seguinte passou. Medido depois: origin/main = HEAD = 44fe06cf e `git log --oneline origin/main..HEAD` VAZIO, com as 2 migrations e a EF v17 no ar acompanhadas dos commits correspondentes. Fica registrado em vez de apagado porque o 49-10-SUMMARY.md chegou a ser escrito afirmando que o criterio estava em aberto e que faltava acao do operador — a correcao e a licao deste arquivo sobre diagnostico plausivel que ninguem mediu, aplicada a si mesma. Nenhuma acao do operador pendente. Razao escrita a mao: `windows fixed` descarta o motivo (PATTERNS 49 §M). | 2026-09-23T03:19:20.062Z | 2026-09-23T03:22:32.029Z |
...
| 67 | 49 | unrun-verify | supabase/tests/p46_purga_smoke.sql | 1532 | p46_purga_smoke (j.2) reprova por estado de fixture-seed que derivou: a vaga 4601d000-...-0003 ('fixture-p46 vaga ativa (sintetica)') esta 'arquivada' desde 2026-08-23 17:47, e a assercao de NAO-VACUIDADE exige 'ativa'. Medido pre-existente: identico com os corpos ANTERIORES ao apply do 49-14. Bloqueia as assercoes (o)/(o.6)/(o.7)/(p) que exercitam o 4o ramo do guard do motor; com a vaga devolvida a 'ativa' dentro de requisicao que aborta, o gate fecha 27/27. | fixed | Fechada em 2026-09-23 pelo plano 49-28: a (j.2) passou a ESTABELECER o estado que mede (UPDATE ... 'ativa' dentro do envelope que aborta em P46B0) em vez de LER o estado vivo de PROD como linha de base. Medido: `node p46apply.cjs run supabase/tests/p46_purga_smoke.sql` sai com exit 0 e devolve P46P_REG_CONTADOR=27 (contador LIDO da execucao, conciliado com o esperado 27 da (z)) com a vaga 4601d000-...-0003 ainda `arquivada`. PROD intocado, medido DEPOIS: vaga `arquivada` com `updated_at` byte a byte o de 2026-08-23 17:47:45.165274, `anon` vendo 2 vagas (as duas reais), zero linha nova em purga_execucoes. Mordida provada por 5 mutacoes + 2 controles: remover o UPDATE de estabelecimento reprova em (j.2) NAO-VACUIDADE. | 2026-09-23T05:25:53.300Z | 2026-09-23T19:15:58.074Z |
| 68 | 49 | stub | supabase/functions/comparativo-candidatos/index.ts | 434 | 49-26: SEXTO sitio da mesma familia, achado pela re-varredura das SETE consumidoras de callAi (o 49-25 varreu cinco e nao alcancou esta). `const ranking = result.parsed ?? null` — NAO ha nenhuma guarda de bloqueio pre-provedor nesta EF: zero ocorrencias de flagged_for_human_review e nenhuma leitura de error_code. Com o teto diario de custo (AI-06) estourado, callAi devolve parsed={recommendation:'hold',flagged_for_human_review:true}, logo `ranking` fica NAO nulo e o stub e (a) inserido em comparativo_solicitado.ranking (jsonb NOT NULL) como se fosse o ranking auditado, e (b) DEVOLVIDO ao RH no payload (`return jsonResponse({ok:true, ranking, ...})`) — um comparativo que nenhum modelo produziu chega a tela como comparativo. provedor_ia/modelo_ia ja saem NULL corretamente, entao a linha de auditoria fica auto-contraditoria: ranking presente, autor ausente. NAO consertado (Scope Boundary: o <measure_first> do 49-26 mandou reportar e nao consertar; EF de outro plano, exige deploy proprio). Conserto: a mesma pergunta estrutural — `!algumProvedorRespondeu(result)` de _shared/resultado-de-provedor.ts, que este plano criou e que esta EF ainda nao importa. | fixed | 49-27 (2026-09-23): consertado e deployado. A EF passou a consumir o predicado ESTRUTURAL `algumProvedorRespondeu` de _shared/resultado-de-provedor.ts (fonte unica do 49-26) antes do caminho de sucesso. Medido ANTES do conserto, com execucao ponta-a-ponta: HTTP 200 com o artefato no corpo, a MESMA forma persistida em comparativo_solicitado.ranking, provedor_ia/modelo_ia NULL (linha auto-contraditoria) e zero chamadas ao provedor. DEPOIS: HTTP 503 error_code=SEM_RESULTADO_IA com motivo (cost_cap_exceeded \| prompt_injection_detected) no corpo, e a linha de auditoria gravada com um marcador de bloqueio explicito ({bloqueado:true, motivo}) — `ranking` e jsonb NOT NULL (medido em PROD: sem default e sem CHECK de forma), entao NULL nao era opcao e `{}` seria indistinguivel de 'nada escrito'. Nenhuma migration: o schema vivo ja permitia registrar o bloqueio. 3 testes novos; os 18 do 49-08 intactos; 5 mutacoes, 5 mordem (M5 reintroduz a forma PROIBIDA — lista literal de codigo de erro — e reprova so o teste da injecao, que e a prova de que a guarda e estrutural). EF v29 ACTIVE verify_jwt=true, marcadores conferidos no bundle PUBLICADO. Linhas ja gravadas com o artefato em PROD: ZERO (as 4 linhas existentes tem a forma real, de 5 chaves) — nada para o 49-12. | 2026-09-23T05:59:25.800Z | 2026-09-23T18:53:30.880Z |
...
| 75 | 49 | unrun-verify | supabase/tests/p46_purga_smoke.sql |  | Asseroes (j.2), (o), (o.6), (o.7) e (p) bloqueadas: o operador RECUSOU abrir a vaga seed 4601d000-...-0003 (fica arquivada). O 4o ramo do guard do motor segue NAO exercitado entrando no 49-19. Conserto e do plano 49-28 (fixture que abre e restaura dentro do proprio envelope). | fixed | Fechada em 2026-09-23 pelo plano 49-28, e sem tocar na vaga: as assercoes (o), (o.6), (o.7) e (p) voltaram a ser ALCANCADAS porque o fail-fast da (j.2) deixou de disparar. Prova: o gate exige exatamente 27 PASS e o run devolve P46P_REG_CONTADOR=27, numero que so e atingivel se cada um dos 27 set_config tiver executado — as 5 assercoes do 46-04 (b, o, o.6, o.7, p) entre eles. A recusa do operador foi HONRADA: a vaga 4601d000-...-0003 segue `arquivada` e `anon` segue vendo 2 vagas, ambos medidos DEPOIS da execucao. O 4o ramo do guard do motor entra no 49-19 EXERCITADO. | 2026-09-23T14:45:13.350Z | 2026-09-23T19:15:58.150Z |
| 76 | 49 | unmet-truth | supabase/migrations/20260922000013_p49_motor_respostas_e_producoes.sql |  | 49-20: as escolhas de multipla escolha da SJT sobrevivem dentro de scores_candidato.metadata->'respostas' (medido: 4 de 5 linhas sjt, com opcao_id/pergunta_id/peso). O recibo promete no item respostas_e_producoes que «as suas respostas das avaliacoes foram apagadas», e a Correcao 14 REJEITOU a opcao (c) («as alternativas que voce marcou ficam»). Nao foi consertado aqui: a migration ja esta aplicada e escriturada, e alargar o escopo do passo de mao unica alem do que o operador enumerou nao e conserto de agente. Conserto cirurgico possivel (metadata - 'respostas', a linha fica), decisao do operador via 49-21. | fixed | 49-21 / D-69 (decisao do operador, 2026-09-23): o passo apagar_respostas_e_producoes passa a remover scores_candidato.metadata -> respostas — a chave sai, a LINHA fica (metadata - 'respostas'), porque metadata e jsonb NOT NULL e carrega composite_0_25 e as notas por dimensao, que sao a prova de que houve avaliacao (ERASE-08 / RNF-07a). Statement (14/14) do mesmo passo, escopado por candidatura_id, com predicado jsonb_exists no WHERE (para ROW_COUNT contar RASPAGENS e nao VISITAS) e contagem propria declarada em 'passos' e no dry-run pela MESMA expressao (chave scores_candidato_respostas_sjt). Migration 20260923000001_p49_motor_respostas_sjt.sql APLICADA e ESCRITURADA (md5 do ledger = md5 do arquivo = 0ed04de22ab3c58f3d294ae745c7d5e8); md5 vivo de anonimizar_candidato = 1d8f96c8f21a755ded0505a0b652113a. Provado por EXECUCAO em p45_motor_exclusao_smoke (B23, quatro metades: nao-vacuidade, a chave sai, motivos_revisao/composite_0_25 FICAM, e as linhas de OUTRAS candidaturas guardam as suas escolhas) e por FORMA em (C3/vii, quatro clausulas acrescentadas ANTES do re-pin). Contador do smoke 36 -> 37. ONZE mutacoes reprovaram, nenhuma chegou a MUTACAO_TERMINOU; M3b (escopo vazado com a contagem honesta) reprovou nomeando antes=4 depois=0 sobre populacao REAL de PROD, e M6c (statement fora do passo com o pin re-carimbado) e a unica que so a rede de forma pega. O recibo passou a apontar o passo novo e a listar scores_candidato.metadata como origem, entao a promessa e o mecanismo dizem a mesma coisa. O motor NAO foi executado contra titular real: config_purga.modo = dry_run, e as 4 linhas reais com a chave continuam intactas em PROD. | 2026-09-23T15:42:18.742Z | 2026-09-23T20:38:48.031Z |
| 77 | 49 | unmet-truth | supabase/migrations/20260922000013_p49_motor_respostas_e_producoes.sql |  | 49-20: resumos do texto removido sobrevivem — redacoes_candidato.texto_hash (NOT NULL, 2 linhas) e input_hash, e entrevista_analises.texto_hash (5 linhas). Nao sao o texto e nao estao entre as 14 origens do recibo, mas permitem CONFIRMAR um texto adivinhado depois de o original ter sido apagado (Art. 12 §1o, meios razoaveis). Registrado, nao consertado: fora do que o operador enumerou no D-48. Avaliar no 49-21. | waived | D-70 (operador, 2026-09-23): residuo ACEITO, nao consertado — waived e nao fixed de proposito. Os tres hashes (redacoes_candidato.texto_hash, .input_hash e entrevista_analises.texto_hash) permanecem depois da exclusao porque sao o vinculo com ai_call_logs que os planos 49-10/49-12 construiram e a chave de idempotencia do D-40; apaga-los desfaria a proveniencia que o JORN-28 exige. O risco e estreito e esta nomeado: exige acesso ao banco E um palpite do texto — permite CONFIRMAR, nunca RECUPERAR. Registrado pelo plano 49-17 (dono nomeado no D-70) na nota da coluna em pii-inventory.yaml, na razao do veredito de export e como chave_tecnica em FORA_DO_RECIBO; o recibo NAO afirma em lugar nenhum que eles foram apagados. Cobertura do 49-17 Task 1: entrevista_analises.texto_hash. As duas de redacoes_candidato entram na Task 3, apos o checkpoint do operador. | 2026-09-23T15:42:18.821Z | 2026-09-23T21:20:08.642Z |
| 78 | 49 | stub | src/features/triagem/components/ComparativoScreen.tsx | 176 | 49-27: a EF passou a recusar o comparativo bloqueado com error_code=SEM_RESULTADO_IA (503) e o motivo no corpo, mas NENHUMA tela traduz esse codigo. O <AsyncState> ramifica a copia so em MIXED_VAGA (override) e AI_UNAVAILABLE (sobrecarga); qualquer outro codigo cai no generico 'Verifique a conexao e tente novamente.' — que e FALSO para um corte de gasto ou uma injecao detectada (a conexao esta boa). O toast do useComparativo mostra 'Nao foi possivel gerar o comparativo. Tente novamente.', que e verdadeiro mas nao diz o motivo. Medido lendo AsyncState.tsx:176, ComparativoScreen.tsx:154 e triagemService.ts RECUSA_COMPARATIVO_COPY. Fora do escopo do 49-27 por files_modified (so a EF) e pelo D-55 (a EF vai primeiro de proposito). Conserto: uma entrada em RECUSA_COMPARATIVO_COPY e um ramo de copia para SEM_RESULTADO_IA — materia do 49-22 / do selo do 49-16. | fixed | Fechada em 2026-09-23 pelo plano 49-22, nas DUAS superficies que a janela nomeia. (1) TELA: ComparativoScreen ganhou o ramo SEM_RESULTADO_IA no errorCopyOverride, e o corpo de erro do AsyncState deixou de dizer "Verifique a conexao e tente novamente" — que era FALSO para um corte de gasto. (2) TOAST: RECUSA_COMPARATIVO_COPY.SEM_RESULTADO_IA, que era verdadeiro mas nao dizia o motivo. As duas frases sao deliberadamente DIFERENTES (uma por medium: toast lido de passagem, corpo de erro lido parado) e o comentario de cada uma nomeia a gemea; o FATO afirmado e o mesmo, e e ele que precisa ser verdade nos dois lugares. Nenhuma das duas nomeia a CAUSA exata (cost_cap_exceeded x prompt_injection_detected): dizer "injecao detectada" ao recrutador confirmaria a um atacante que a defesa disparou — ha assercao NEGATIVA disso em triagemService.test.ts. Provas: 4 testes novos (3 em ComparativoScreen.test.tsx, incluindo o de que um codigo DESCONHECIDO continua caindo no generico, 1 em triagemService.test.ts); marcadores lidos de volta de https://rh.beautysmile.com.br — "Nenhum modelo de IA chegou a ser consultado" no chunk lazy ComparativoScreen-ZPSuNWY1.js e "Nenhum modelo de IA foi consultado" no indice eager index-C55SemLW.js. Commits dd0bb32a (tela) e cb397303 (toast), ambos em origin/main. NOTA DE INSTRUMENTO: esta nota foi escrita A MAO porque `windows fixed` foi disparado por engano numa sonda de descoberta de assinatura e gravou status=fixed com resolucao VAZIA; o comando recusa re-fixar uma janela ja fixed, e uma janela fixed sem razao escrita e o defeito que o commit e83a96dd corrigiu nas janelas 60 e 63. | 2026-09-23T18:55:05.134Z | 2026-09-23T19:55:09.067Z |
| 79 | 49 | deviation | supabase/tests/p46_purga_smoke.sql | 1690 | FORMA-FOTOGRAFIA registrada e NAO consertada pelo 49-28: a pre-condicao da (k) exige v_k4_hist=0 e v_k4_dec=0 LENDO o estado vivo da fixture pos3 (4601c000-...-0003) como linha de base — a mesma classe da (j.2). NAO consertada porque o estabelecimento seria DESTRUTIVO (DELETE em historico_candidatura + data_decisao_final := NULL) e porque, se pos3 ganhou historico, o degrau CORRETO da escada da data-ancora MUDA: forcar o degrau apagando historico manufaturaria o cenario em vez de escolher a fixture. Conserto honesto = fixture criada DENTRO do envelope, que e redesenho. Hoje PASSA (medido 2026-09-23). | open |  | 2026-09-23T19:17:59.584Z |  |
| 80 | 49 | deviation | supabase/tests/p46_purga_smoke.sql | 2452 | FORMA-LISTA-LITERAL registrada e NAO consertada pelo 49-28: a (a) confere PRESENCA dos 3 agendamentos herdados iterando sobre c_herdados (lista literal) com NOT EXISTS. E o ponto cego que o CLAUDE.md nomeia em §Portoes: um job NOVO em cron.job fica FORA da vigilancia e o portao segue VERDE. Escopo deliberado para o que ela AFIRMA (os 3 declarados continuam existindo) e cega para ACRESCIMO. O inventario por contagem pertence ao p42_invent05_cron_smoke — fora do escopo deste plano, que toca um arquivo. | open |  | 2026-09-23T19:18:26.546Z |  |
| 81 | 49 | unmet-truth | docs/compliance/pii-inventory.yaml |  | 49-21 / D-66: a lista tabelas_sem_pii_titular do inventario continua com analise_candidato_vaga e entrevista_guias, e MEDIDO em 2026-09-23 as duas carregam o primeiro nome do titular em texto livre (13/24 analises em resumo_cv/resumo_respostas/gaps/pontos_fortes, 2/5 guias no jsonb guia). anonimizar_candidato NAO as desidentifica (position() = 0 nas tres colunas do corpo vivo). O operador decidiu no D-66 que a CLASSIFICACAO e corrigida nesta fase e que o APAGAMENTO e escopo novo (§Deferred do CONTEXT). Nao movidas para tabelas: no 49-21 porque a regra de COBERTURA do gen-recibo-exclusao obrigaria um veredito por coluna, e o unico veredito honesto hoje seria uma linha NOVA dizendo ao titular que o seu nome continua nesses textos — copy de produto, decisao do operador e nao da engenharia. O que FOI feito: a nota da secao e as duas entradas passaram a registrar a medicao e a ressalva ABERTA, para que o documento deixe de afirmar ausencia de PII. Decisao pendente: (a) o motor desidentificar as duas colunas e entao mover as tabelas para tabelas:, ou (b) copy nova no recibo assumindo o residuo. | open |  | 2026-09-23T20:40:45.842Z |  |
| 82 | 49 | unmet-truth | docs/compliance/sql/gen-recibo-exclusao.cjs |  | 49-21 (medido, NAO consertado): o recibo ESCONDE um apagamento que o motor faz, na linha da justificativa da decisao. MEDIDO no corpo vivo de anonimizar_candidato (migration 20260923000001, passo tombstone_decisao_final): decisao_final.justificativa e decisao_final_historico.justificativa recebem um VALOR FIXO no lugar do que o recrutador escreveu, e esse valor diz, com estas palavras, que o texto original foi removido (tratamento D-45-02/03 da Phase 45). O recibo, porem, diz na coluna «sai» que «o texto continua guardado» (ligacao_com_a_justificativa) e na coluna «mantem» que «fica guardada» (justificativa_do_recrutador, uma das TRES linhas obrigatorias da UI-SPEC regra 4). Para as outras duas origens da mesma linha — candidaturas.motivo_rejeicao e avaliacoes_rh.justificativa_recomendacao — a frase e VERDADEIRA: o motor nao as toca (position() = 0 nas duas). A direcao do erro e a segura (o recibo diz MENOS do que o motor apaga), e por isso a classificacao no inventario segue preservar_com_ressalva. NAO consertado aqui: e defeito PRE-EXISTENTE da Phase 45 (copy contra motor da propria Phase 45), nao esta na tabela C4 do 49-14/49-20, e reescrever uma das tres linhas obrigatorias e copy de produto com peso legal (prova de nao-discriminacao) — decisao do operador. Consequencia pratica no 49-21: revisao_resultado (D-60) NAO foi posto nessas duas linhas; ganhou item proprio (resposta_ao_seu_pedido_de_revisao) com texto que descreve o que o motor faz, para nao herdar a afirmacao falsa. | fixed | 49-17 Task 3 (2026-09-24): CONSERTADO no gerador, e o conserto foi conferido no ARTEFATO PUBLICADO, nao no disco. A linha obrigatoria justificativa_do_recrutador passou a DISTINGUIR as suas tres origens em vez de fazer uma afirmacao unica sobre as tres: candidaturas.motivo_rejeicao e avaliacoes_rh.justificativa_recomendacao continuam guardadas (re-medido em 2026-09-24 no corpo vivo de anonimizar_candidato, md5 1d8f96c8f21a755ded0505a0b652113a / 78301 octetos: position() = 0 para as duas, e para a propria tabela avaliacoes_rh), enquanto no campo da decisao final o que o recrutador escreveu e trocado por um aviso padrao por um UPDATE INCONDICIONAL do passo tombstone_decisao_final. A linha continua existindo, continua em OBRIGATORIAS_MANTEM e continua reivindicando as mesmas quatro colunas: mudou o que ela DIZ, nao se ela existe, e as outras duas linhas obrigatorias nao foram tocadas. A classificacao no inventario segue preservar_com_ressalva — faltava distinguir origem, nao reclassificar coluna. Consertada tambem a OUTRA metade que esta entrada acusa, ligacao_com_a_justificativa, na coluna «sai»: as origens dela sao SO o par decisao_final/decisao_final_historico, ou seja a afirmacao era falsa para TODAS as origens daquela linha, e nao para uma entre tres — fechar a janela deixando aquela de pe seria fechar no papel com o defeito vivo um item acima, na mesma tela. O valor fixo NAO e citado em lugar nenhum do codigo (49-PATTERNS §K). PROVA NO AR (49-PATTERNS §N, um marcador por chamada): executar-direito-titular redeployada (version=11, ACTIVE, verify_jwt=true) e o bundle vivo baixado da Management API (640005 octetos) contem 3x «trocado por um aviso padrao» e 3x «a recomendacao escrita por quem avaliou», e ZERO ocorrencia das duas afirmacoes retiradas. | 2026-09-23T20:41:02.174Z | 2026-09-24T05:22:08.682Z |
...
[
  ...
  },
  {
    ...
    "status": "fixed",
    "reason": "Consertado pelo plano 49-25 (EF gerar-guia-entrevista v20, ACTIVE em PROD): o helper guiaDeResultado() passou a perguntar pelo PROVEDOR (provider === 'none' => nenhum modelo respondeu) em vez de testar parsed == null, de modo que o teto de custo AI-06, a deteccao de injecao RF-PL-18 e todo bloqueio pre-provedor futuro ficam cobertos por construcao, sem lista literal de error_code. A linha passa a sair com incompleto:true e o error_code em flags; as perguntas origem:'manual' seguem preservadas pelo merge ENTREV-08, asserido. Provado por 5 mutacoes, todas mordendo. Medido em PROD: ZERO linhas ja gravadas no estado defeituoso e ZERO linhas provider='none' em ai_call_logs desde sempre (maior gasto diario por vaga da historia: US$ 0,3096 contra teto de US$ 50), logo o defeito era LATENTE e nao houve nada retroativo a corrigir. Razao escrita a mao: `windows fixed` aceita o motivo e o descarta (PATTERNS 49 §M).",
  ...
  },
  {
    ...
    "status": "fixed",
    "reason": "Resolvido no proprio plano 49-10, em 2026-09-23. A primeira tentativa de `git push origin main` foi negada pelo ambiente e este registro foi aberto enquanto era verdadeiro; a tentativa seguinte passou. Medido depois: origin/main = HEAD = 44fe06cf e `git log --oneline origin/main..HEAD` VAZIO, com as 2 migrations e a EF v17 no ar acompanhadas dos commits correspondentes. Fica registrado em vez de apagado porque o 49-10-SUMMARY.md chegou a ser escrito afirmando que o criterio estava em aberto e que faltava acao do operador — a correcao e a licao deste arquivo sobre diagnostico plausivel que ninguem mediu, aplicada a si mesma. Nenhuma acao do operador pendente. Razao escrita a mao: `windows fixed` descarta o motivo (PATTERNS 49 §M).",
  ...
  },
  {
    ...
    "status": "fixed",
    "reason": "Fechada em 2026-09-23 pelo plano 49-28: a (j.2) passou a ESTABELECER o estado que mede (UPDATE ... 'ativa' dentro do envelope que aborta em P46B0) em vez de LER o estado vivo de PROD como linha de base. Medido: `node p46apply.cjs run supabase/tests/p46_purga_smoke.sql` sai com exit 0 e devolve P46P_REG_CONTADOR=27 (contador LIDO da execucao, conciliado com o esperado 27 da (z)) com a vaga 4601d000-...-0003 ainda `arquivada`. PROD intocado, medido DEPOIS: vaga `arquivada` com `updated_at` byte a byte o de 2026-08-23 17:47:45.165274, `anon` vendo 2 vagas (as duas reais), zero linha nova em purga_execucoes. Mordida provada por 5 mutacoes + 2 controles: remover o UPDATE de estabelecimento reprova em (j.2) NAO-VACUIDADE.",
    "recorded_at": "2026-09-23T05:25:53.300Z",
    "resolved_at": "2026-09-23T19:15:58.074Z",
    "milestone": "v8.0"
  },
  {
    "id": 68,
    ...
    "status": "fixed",
    "reason": "49-27 (2026-09-23): consertado e deployado. A EF passou a consumir o predicado ESTRUTURAL `algumProvedorRespondeu` de _shared/resultado-de-provedor.ts (fonte unica do 49-26) antes do caminho de sucesso. Medido ANTES do conserto, com execucao ponta-a-ponta: HTTP 200 com o artefato no corpo, a MESMA forma persistida em comparativo_solicitado.ranking, provedor_ia/modelo_ia NULL (linha auto-contraditoria) e zero chamadas ao provedor. DEPOIS: HTTP 503 error_code=SEM_RESULTADO_IA com motivo (cost_cap_exceeded | prompt_injection_detected) no corpo, e a linha de auditoria gravada com um marcador de bloqueio explicito ({bloqueado:true, motivo}) — `ranking` e jsonb NOT NULL (medido em PROD: sem default e sem CHECK de forma), entao NULL nao era opcao e `{}` seria indistinguivel de 'nada escrito'. Nenhuma migration: o schema vivo ja permitia registrar o bloqueio. 3 testes novos; os 18 do 49-08 intactos; 5 mutacoes, 5 mordem (M5 reintroduz a forma PROIBIDA — lista literal de codigo de erro — e reprova so o teste da injecao, que e a prova de que a guarda e estrutural). EF v29 ACTIVE verify_jwt=true, marcadores conferidos no bundle PUBLICADO. Linhas ja gravadas com o artefato em PROD: ZERO (as 4 linhas existentes tem a forma real, de 5 chaves) — nada para o 49-12.",
    "recorded_at": "2026-09-23T05:59:25.800Z",
    "resolved_at": "2026-09-23T18:53:30.880Z",
    "milestone": "v8.0"
  },
  {
    "id": 69,
  ...
  },
  {
    ...
    "status": "fixed",
    "reason": "Fechada em 2026-09-23 pelo plano 49-28, e sem tocar na vaga: as assercoes (o), (o.6), (o.7) e (p) voltaram a ser ALCANCADAS porque o fail-fast da (j.2) deixou de disparar. Prova: o gate exige exatamente 27 PASS e o run devolve P46P_REG_CONTADOR=27, numero que so e atingivel se cada um dos 27 set_config tiver executado — as 5 assercoes do 46-04 (b, o, o.6, o.7, p) entre eles. A recusa do operador foi HONRADA: a vaga 4601d000-...-0003 segue `arquivada` e `anon` segue vendo 2 vagas, ambos medidos DEPOIS da execucao. O 4o ramo do guard do motor entra no 49-19 EXERCITADO.",
    "recorded_at": "2026-09-23T14:45:13.350Z",
    "resolved_at": "2026-09-23T19:15:58.150Z",
    "milestone": "v8.0"
  },
  {
    "id": 76,
    ...
    "status": "fixed",
    "reason": "49-21 / D-69 (decisao do operador, 2026-09-23): o passo apagar_respostas_e_producoes passa a remover scores_candidato.metadata -> respostas — a chave sai, a LINHA fica (metadata - 'respostas'), porque metadata e jsonb NOT NULL e carrega composite_0_25 e as notas por dimensao, que sao a prova de que houve avaliacao (ERASE-08 / RNF-07a). Statement (14/14) do mesmo passo, escopado por candidatura_id, com predicado jsonb_exists no WHERE (para ROW_COUNT contar RASPAGENS e nao VISITAS) e contagem propria declarada em 'passos' e no dry-run pela MESMA expressao (chave scores_candidato_respostas_sjt). Migration 20260923000001_p49_motor_respostas_sjt.sql APLICADA e ESCRITURADA (md5 do ledger = md5 do arquivo = 0ed04de22ab3c58f3d294ae745c7d5e8); md5 vivo de anonimizar_candidato = 1d8f96c8f21a755ded0505a0b652113a. Provado por EXECUCAO em p45_motor_exclusao_smoke (B23, quatro metades: nao-vacuidade, a chave sai, motivos_revisao/composite_0_25 FICAM, e as linhas de OUTRAS candidaturas guardam as suas escolhas) e por FORMA em (C3/vii, quatro clausulas acrescentadas ANTES do re-pin). Contador do smoke 36 -> 37. ONZE mutacoes reprovaram, nenhuma chegou a MUTACAO_TERMINOU; M3b (escopo vazado com a contagem honesta) reprovou nomeando antes=4 depois=0 sobre populacao REAL de PROD, e M6c (statement fora do passo com o pin re-carimbado) e a unica que so a rede de forma pega. O recibo passou a apontar o passo novo e a listar scores_candidato.metadata como origem, entao a promessa e o mecanismo dizem a mesma coisa. O motor NAO foi executado contra titular real: config_purga.modo = dry_run, e as 4 linhas reais com a chave continuam intactas em PROD.",
    "recorded_at": "2026-09-23T15:42:18.742Z",
    "resolved_at": "2026-09-23T20:38:48.031Z",
    "milestone": "v8.0"
  },
  {
    "id": 77,
    ...
    "status": "waived",
    "reason": "D-70 (operador, 2026-09-23): residuo ACEITO, nao consertado — waived e nao fixed de proposito. Os tres hashes (redacoes_candidato.texto_hash, .input_hash e entrevista_analises.texto_hash) permanecem depois da exclusao porque sao o vinculo com ai_call_logs que os planos 49-10/49-12 construiram e a chave de idempotencia do D-40; apaga-los desfaria a proveniencia que o JORN-28 exige. O risco e estreito e esta nomeado: exige acesso ao banco E um palpite do texto — permite CONFIRMAR, nunca RECUPERAR. Registrado pelo plano 49-17 (dono nomeado no D-70) na nota da coluna em pii-inventory.yaml, na razao do veredito de export e como chave_tecnica em FORA_DO_RECIBO; o recibo NAO afirma em lugar nenhum que eles foram apagados. Cobertura do 49-17 Task 1: entrevista_analises.texto_hash. As duas de redacoes_candidato entram na Task 3, apos o checkpoint do operador.",
    "recorded_at": "2026-09-23T15:42:18.821Z",
    "resolved_at": "2026-09-23T21:20:08.642Z",
    "milestone": "v8.0"
  },
  {
    "id": 78,
    "kind": "stub",
    "phase": "49",
    "file": "src/features/triagem/components/ComparativoScreen.tsx",
    "line": 176,
    "description": "49-27: a EF passou a recusar o comparativo bloqueado com error_code=SEM_RESULTADO_IA (503) e o motivo no corpo, mas NENHUMA tela traduz esse codigo. O <AsyncState> ramifica a copia so em MIXED_VAGA (override) e AI_UNAVAILABLE (sobrecarga); qualquer outro codigo cai no generico 'Verifique a conexao e tente novamente.' — que e FALSO para um corte de gasto ou uma injecao detectada (a conexao esta boa). O toast do useComparativo mostra 'Nao foi possivel gerar o comparativo. Tente novamente.', que e verdadeiro mas nao diz o motivo. Medido lendo AsyncState.tsx:176, ComparativoScreen.tsx:154 e triagemService.ts RECUSA_COMPARATIVO_COPY. Fora do escopo do 49-27 por files_modified (so a EF) e pelo D-55 (a EF vai primeiro de proposito). Conserto: uma entrada em RECUSA_COMPARATIVO_COPY e um ramo de copia para SEM_RESULTADO_IA — materia do 49-22 / do selo do 49-16.",
    "status": "fixed",
    "reason": "Fechada em 2026-09-23 pelo plano 49-22, nas DUAS superficies que a janela nomeia. (1) TELA: ComparativoScreen ganhou o ramo SEM_RESULTADO_IA no errorCopyOverride, e o corpo de erro do AsyncState deixou de dizer \"Verifique a conexao e tente novamente\" — que era FALSO para um corte de gasto. (2) TOAST: RECUSA_COMPARATIVO_COPY.SEM_RESULTADO_IA, que era verdadeiro mas nao dizia o motivo. As duas frases sao deliberadamente DIFERENTES (uma por medium: toast lido de passagem, corpo de erro lido parado) e o comentario de cada uma nomeia a gemea; o FATO afirmado e o mesmo, e e ele que precisa ser verdade nos dois lugares. Nenhuma das duas nomeia a CAUSA exata (cost_cap_exceeded x prompt_injection_detected): dizer \"injecao detectada\" ao recrutador confirmaria a um atacante que a defesa disparou — ha assercao NEGATIVA disso em triagemService.test.ts. Provas: 4 testes novos (3 em ComparativoScreen.test.tsx, incluindo o de que um codigo DESCONHECIDO continua caindo no generico, 1 em triagemService.test.ts); marcadores lidos de volta de https://rh.beautysmile.com.br — \"Nenhum modelo de IA chegou a ser consultado\" no chunk lazy ComparativoScreen-ZPSuNWY1.js e \"Nenhum modelo de IA foi consultado\" no indice eager index-C55SemLW.js. Commits dd0bb32a (tela) e cb397303 (toast), ambos em origin/main. NOTA DE INSTRUMENTO: esta nota foi escrita A MAO porque `windows fixed` foi disparado por engano numa sonda de descoberta de assinatura e gravou status=fixed com resolucao VAZIA; o comando recusa re-fixar uma janela ja fixed, e uma janela fixed sem razao escrita e o defeito que o commit e83a96dd corrigiu nas janelas 60 e 63.",
    "recorded_at": "2026-09-23T18:55:05.134Z",
    "resolved_at": "2026-09-23T19:55:09.067Z",
    "milestone": "v8.0"
  },
  {
    "id": 79,
    ...
    "file": "supabase/tests/p46_purga_smoke.sql",
    "line": 1690,
    "description": "FORMA-FOTOGRAFIA registrada e NAO consertada pelo 49-28: a pre-condicao da (k) exige v_k4_hist=0 e v_k4_dec=0 LENDO o estado vivo da fixture pos3 (4601c000-...-0003) como linha de base — a mesma classe da (j.2). NAO consertada porque o estabelecimento seria DESTRUTIVO (DELETE em historico_candidatura + data_decisao_final := NULL) e porque, se pos3 ganhou historico, o degrau CORRETO da escada da data-ancora MUDA: forcar o degrau apagando historico manufaturaria o cenario em vez de escolher a fixture. Conserto honesto = fixture criada DENTRO do envelope, que e redesenho. Hoje PASSA (medido 2026-09-23).",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-23T19:17:59.584Z",
    "resolved_at": null,
    "milestone": "v8.0"
  },
  {
    ...
    "file": "supabase/tests/p46_purga_smoke.sql",
    "line": 2452,
    "description": "FORMA-LISTA-LITERAL registrada e NAO consertada pelo 49-28: a (a) confere PRESENCA dos 3 agendamentos herdados iterando sobre c_herdados (lista literal) com NOT EXISTS. E o ponto cego que o CLAUDE.md nomeia em §Portoes: um job NOVO em cron.job fica FORA da vigilancia e o portao segue VERDE. Escopo deliberado para o que ela AFIRMA (os 3 declarados continuam existindo) e cega para ACRESCIMO. O inventario por contagem pertence ao p42_invent05_cron_smoke — fora do escopo deste plano, que toca um arquivo.",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-23T19:18:26.546Z",
    "resolved_at": null,
    "milestone": "v8.0"
  },
  {
    ...
    "line": null,
    "description": "49-21 / D-66: a lista tabelas_sem_pii_titular do inventario continua com analise_candidato_vaga e entrevista_guias, e MEDIDO em 2026-09-23 as duas carregam o primeiro nome do titular em texto livre (13/24 analises em resumo_cv/resumo_respostas/gaps/pontos_fortes, 2/5 guias no jsonb guia). anonimizar_candidato NAO as desidentifica (position() = 0 nas tres colunas do corpo vivo). O operador decidiu no D-66 que a CLASSIFICACAO e corrigida nesta fase e que o APAGAMENTO e escopo novo (§Deferred do CONTEXT). Nao movidas para tabelas: no 49-21 porque a regra de COBERTURA do gen-recibo-exclusao obrigaria um veredito por coluna, e o unico veredito honesto hoje seria uma linha NOVA dizendo ao titular que o seu nome continua nesses textos — copy de produto, decisao do operador e nao da engenharia. O que FOI feito: a nota da secao e as duas entradas passaram a registrar a medicao e a ressalva ABERTA, para que o documento deixe de afirmar ausencia de PII. Decisao pendente: (a) o motor desidentificar as duas colunas e entao mover as tabelas para tabelas:, ou (b) copy nova no recibo assumindo o residuo.",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-23T20:40:45.842Z",
    "resolved_at": null,
    "milestone": "v8.0"
  },
  {
    ...
    "file": "docs/compliance/sql/gen-recibo-exclusao.cjs",
    "line": null,
    "description": "49-21 (medido, NAO consertado): o recibo ESCONDE um apagamento que o motor faz, na linha da justificativa da decisao. MEDIDO no corpo vivo de anonimizar_candidato (migration 20260923000001, passo tombstone_decisao_final): decisao_final.justificativa e decisao_final_historico.justificativa recebem um VALOR FIXO no lugar do que o recrutador escreveu, e esse valor diz, com estas palavras, que o texto original foi removido (tratamento D-45-02/03 da Phase 45). O recibo, porem, diz na coluna «sai» que «o texto continua guardado» (ligacao_com_a_justificativa) e na coluna «mantem» que «fica guardada» (justificativa_do_recrutador, uma das TRES linhas obrigatorias da UI-SPEC regra 4). Para as outras duas origens da mesma linha — candidaturas.motivo_rejeicao e avaliacoes_rh.justificativa_recomendacao — a frase e VERDADEIRA: o motor nao as toca (position() = 0 nas duas). A direcao do erro e a segura (o recibo diz MENOS do que o motor apaga), e por isso a classificacao no inventario segue preservar_com_ressalva. NAO consertado aqui: e defeito PRE-EXISTENTE da Phase 45 (copy contra motor da propria Phase 45), nao esta na tabela C4 do 49-14/49-20, e reescrever uma das tres linhas obrigatorias e copy de produto com peso legal (prova de nao-discriminacao) — decisao do operador. Consequencia pratica no 49-21: revisao_resultado (D-60) NAO foi posto nessas duas linhas; ganhou item proprio (resposta_ao_seu_pedido_de_revisao) com texto que descreve o que o motor faz, para nao herdar a afirmacao falsa.",
    "status": "fixed",
    "reason": "49-17 Task 3 (2026-09-24): CONSERTADO no gerador, e o conserto foi conferido no ARTEFATO PUBLICADO, nao no disco. A linha obrigatoria justificativa_do_recrutador passou a DISTINGUIR as suas tres origens em vez de fazer uma afirmacao unica sobre as tres: candidaturas.motivo_rejeicao e avaliacoes_rh.justificativa_recomendacao continuam guardadas (re-medido em 2026-09-24 no corpo vivo de anonimizar_candidato, md5 1d8f96c8f21a755ded0505a0b652113a / 78301 octetos: position() = 0 para as duas, e para a propria tabela avaliacoes_rh), enquanto no campo da decisao final o que o recrutador escreveu e trocado por um aviso padrao por um UPDATE INCONDICIONAL do passo tombstone_decisao_final. A linha continua existindo, continua em OBRIGATORIAS_MANTEM e continua reivindicando as mesmas quatro colunas: mudou o que ela DIZ, nao se ela existe, e as outras duas linhas obrigatorias nao foram tocadas. A classificacao no inventario segue preservar_com_ressalva — faltava distinguir origem, nao reclassificar coluna. Consertada tambem a OUTRA metade que esta entrada acusa, ligacao_com_a_justificativa, na coluna «sai»: as origens dela sao SO o par decisao_final/decisao_final_historico, ou seja a afirmacao era falsa para TODAS as origens daquela linha, e nao para uma entre tres — fechar a janela deixando aquela de pe seria fechar no papel com o defeito vivo um item acima, na mesma tela. O valor fixo NAO e citado em lugar nenhum do codigo (49-PATTERNS §K). PROVA NO AR (49-PATTERNS §N, um marcador por chamada): executar-direito-titular redeployada (version=11, ACTIVE, verify_jwt=true) e o bundle vivo baixado da Management API (640005 octetos) contem 3x «trocado por um aviso padrao» e 3x «a recomendacao escrita por quem avaliou», e ZERO ocorrencia das duas afirmacoes retiradas.",
    "recorded_at": "2026-09-23T20:41:02.174Z",
    "resolved_at": "2026-09-24T05:22:08.682Z",
    "milestone": "v8.0"
  },
  {
    "id": 83,
    ...
    "file": "docs/compliance/pii-inventory.yaml",
    "line": null,
    "description": "49-17: a MEDICAO do D-66 na WINDOWS 81 esta errada nos dois sentidos, re-medida em 2026-09-23 (so leitura). (1) entrevista_guias: ZERO de 5 guias contem o primeiro nome do titular, nao 2 de 5. Os dois supostos achados sao a palavra comum portuguesa candidato dentro do rationale do proprio guia (posicoes 475 e 629, texto extraido e lido), e casam por acidente porque a conta de teste se chama Candidato Funil Teste. Um probe por nome contra uma base cujo titular se chama como um substantivo comum produz falso positivo. (2) analise_candidato_vaga: 9 de 25 linhas contem o primeiro nome ATUAL, nao 13 de 24 — e esse 9 e um LIMITE INFERIOR, porque casar contra o nome atual e estruturalmente cego ao caso que mais importa: o unico titular que de fato exerceu a exclusao teve o nome substituido em candidatos, e por isso nenhum probe por nome encontra o que sobrou no texto. Medido diretamente nessa linha: resumo_cv guarda o nome COMPLETO original (tres partes, 132 caracteres) depois da exclusao concluida. anonimizar_candidato nao cita analise_candidato_vaga nem nenhuma das tres colunas no corpo vivo (md5 1d8f96c8f21a755ded0505a0b652113a, 78301 octetos). Consequencia para a decisao do operador: o item avaliacoes_e_analises do recibo promete a esse titular que as analises ficaram guardadas sem ligacao com voce, e para essa linha a frase e FALSA hoje. Nao consertado: a escolha entre desidentificar no motor e copy nova e do operador, no checkpoint do 49-17. DONO: plano 49-29 (onda 8), criado pelo operador em 2026-09-23 ao escolher a opcao (c) do checkpoint do 49-17 — desidentificar no motor em vez de enfraquecer o recibo. Consequencia para o 49-17: o texto do item avaliacoes_e_analises NAO foi alterado (a promessa «sem ligacao com voce» fica de pe porque o motor vai passar a cumpri-la) e as duas tabelas NAO sairam de tabelas_sem_pii_titular — a reclassificacao vai com o 49-29, que e quem conserta o motor. Esta entrada fica open ate lá. Acrescentado em 2026-09-24: entrevista_guias tem ZERO guias pertencentes a titular anonimizado (0 de 5, medido), logo o ponto cego do probe por nome tambem nao a alcanca hoje; a ressalva dela e ESTRUTURAL (o guia e derivado do curriculo), nao medida.",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-23T21:20:26.749Z",
    "resolved_at": null,
    "milestone": "v8.0"
  },
  {
    ...
    "file": ".planning/WINDOWS.md",
    "line": null,
    "description": "SONDA DE COMPARADOR (49-17, paragrafo M do 49-PATTERNS): esta entrada existe para exercitar o comparador FORTE do ledger depois de um waive e um append. Se ela aparece, a tabela do topo e o bloco JSON concordam. Sera marcada waived no mesmo commit.",
    "status": "waived",
    "reason": "Sonda de comparador do plano 49-17: cumpriu o proposito (o append aceitou, logo tabela e JSON concordam). Sem defeito de produto por tras.",
    "recorded_at": "2026-09-23T21:20:37.103Z",
    "resolved_at": "2026-09-23T21:20:47.261Z",
    "milestone": "v8.0"
  },
  {
    "id": 85,
    ...
    "line": null,
    "description": "49-17 Task 3 (2026-09-24), ACHADO PRE-EXISTENTE registrado e NAO consertado: a view public.v_analises_presas e a forma perigosa que a memoria do projeto nomeia — security_invoker NAO setado (semantica de DEFINIDOR) e dono postgres, logo ela IGNORA RLS — e tem SELECT concedido a authenticated. Lida a definicao: expoe candidatura_id, vaga_id, vaga_slug, data_candidatura, situacao, a mensagem de erro da analise e o tempo parada. NENHUMA coluna da Phase 49, nenhum nome, nenhum CPF, e anon nao a alcanca (medido com SET LOCAL ROLE anon no 49-17 Task 1). O que ela permite: qualquer candidato AUTENTICADO listar as candidaturas travadas do sistema inteiro, inclusive de outras pessoas. Fora do escopo do 49-17 (o plano decide vereditos de export, nao conserta views), e registrado aqui porque vivia so no SUMMARY de um plano — o mesmo defeito de escrituracao que o 49-PATTERNS §K documenta num nivel acima. Conserto: ALTER VIEW public.v_analises_presas SET (security_invoker = true), com re-medicao de quem a le antes e depois.",
    "status": "open",
    "reason": "",
