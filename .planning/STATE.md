...
- [Phase 48]: 48-10: liberar a avaliação cognitiva avisa o candidato (evento cognitivo_liberado, chave {candidatura}:cognitivo_liberado:{ciclo}); só o INSERT e a re-liberação após revogação despacham — clicar Liberar numa liberação vigente e revogar não — D-22; o upsert de liberar_cognitivo sempre recarimba liberado_em, então o guard de transição no corpo do trigger separa aviso novo de recarimbo
- [Phase 48]: 48-11: revertida REABRE (decisao_final/em_analise, data_decisao_final NULL, prazo = fim do 10o dia em SP; A3/A4) — nunca aprova (D-01)
...
- 49-18 Task 2 PENDENTE (causa FECHADA, não reabrir investigação de ambiente): a sessão 1 da jornada NÃO foi executada — o que chegou como "sessão 1 feita" era o roteiro, não o executado (confirmado pelo operador em 2026-09-26). Medido e reconfirmado pelo orquestrador: zero linhas em qualquer tabela após T0=2026-09-26T18:44:16Z, +claude7/8/9 inexistentes em candidatos e auth.users, nenhum login no projeto desde 2026-09-22. NÃO é divergência de ambiente/projeto/alias. Para fechar: executar a sessão 1 com o roteiro da §4b do 49-PROVA-PROD.md (5 candidaturas abertas na Social Media — +claude7/8/9 a criar sem a opção que elimina —, porque o tooltip do teto só aparece em 5ª linha não encerrada), relatar (a)-(f) com conteúdo, e a prova voltar com as 19 colunas true. T0 PRESERVADO.
